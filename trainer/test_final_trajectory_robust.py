...
class Trainer:
    ...
    def fit(self):
        ...
        # ---------------------------------------------------------
        # Save robust evaluation result to CSV
        # ---------------------------------------------------------
        os.makedirs("results/robust", exist_ok=True)

        csv_path = os.path.join("results", "robust", f"{self.config.data_scene}.csv")
        ...
        print(f"[Saved robust CSV] {csv_path}")
    ...
    def evaluate(self, dataset):
        ...
        # Clean original past and valid mask are important for robust analysis.
        save_past_rel_clean = []
        save_past_abs_clean = []
        save_valid_mask = []
        ...
        with torch.no_grad():
            for _, (trajectory, mask, initial_pos, seq_start_end) in enumerate(dataset):
                ...
                # The PPT model also receives abs_past.
                ...
                if save_enabled:
                    ...
                    # past_rel is the corrupted input actually fed into PPT.
                    ...
                    # past_abs is the corrupted input actually fed into PPT.
            ...
            if save_enabled:
                if len(save_past_rel) == 0:
                    ...
                    pred_tag = getattr(self.config, "pred_tag", "robust")
                    ...
                    np.savez_compressed(
                        ...
                        # Corrupted past actually seen by PPT.
